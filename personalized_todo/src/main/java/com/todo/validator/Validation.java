...
public class Validation {
	...
	public static boolean isLoginUser(String name, String password) {
		if ((name.equals("validUser")) && (password.equals("password"))) {
	...
	public static boolean isValidTask() {
		 ...
		 * (task.getAssignedTo().trim().length() > 3) &&
		 * (task.getTaskStatus().trim().length() > 3)) {
		 * System.out.println(task.getTaskStatus());
		 * 
		 * System.out.println("task data validated...completed"); return true;
		 * 
		 * } else { System.out.println("Task data validation failed"); return false; }
		 */
		return false;
	}
}
